...
export async function getServerSideProps(){
    ...
    pData = pData.filter(pack =>
        pack.destination_search.split(',').map(keyword => keyword.trim()).includes('Group')
        );
    console.log(pData);
    return {
        props: {
...
export default function test({pData}) {
    ...
    return (
        <div>
            <Head>
                <title>Group Tour Packages: Book @ 𝗨𝗽𝘁𝗼 𝟱𝟬% 𝗢𝗳𝗳</title>
                <meta name="description" content="Elevate group travel experiences! Discover a variety of group tour packages for seamless adventures. Book now for memorable journey with your squads." />
                <link rel="icon" href="/icon.png" />

            </Head>
            {/* <HomeModal ImageLink="/images/popup/Winter.webp" alt="Winter Tour Packages" /> */}

            <section className="breadcrumb-main pb-6 pt-10" style={{ backgroundImage: "url(/images/banner_images/group.jpg)" }}>
                        ...
                        <div className="breadcrumb-content text-center">
                            <nav aria-label="breadcrumb" className="d-block">
                                <ul className="breadcrumb banner-content">
                                    ...
                                    <h1 className='text-white' id="tourPackagesHeading">Group Tour Packages</h1><br />
