...
export async function getServerSideProps(context) {
    ...
    // const jsonDirectory = path.join(process.cwd(), '../../public/output.json');
    // const response = await fetch(jsonDirectory); // Path to your JSON file
    // const jsonData = await response.json();
    var tourData = jsonData.find(tour => tour.title.toLowerCase() === formattedTitle) || {};
    // tourData = JSON.stringify(tourData);
    // tourData = tourData.replace(/\\u0095/g, '&bull;');
...
export default function test({ data22, tourData, place }) {
    ...
    return (
        <div>
            ...
            <section className="breadcrumb-main pb-5 pt-12" style={{ backgroundImage: `url(/images/banner_images/${place.toLowerCase()}.webp)` }} alt={`${place} Tour Packages`}>
